(async () => {
    ...
    root.innerHTML = `
        <style>
            #__raregems_collection_widget {
                ...
                font-family: system-ui; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; line-height: 1; text-transform: uppercase; cursor: pointer;
            }
            ...
            #__raregems_collection_widget .__rg_item { flex: 1; padding: 10px; text-align: center; background: ${dark ? '#212529' : '#dee2e6'}; border-radius: 8px; }
            #__raregems_collection_widget .__rg_name { font-size: 10px; text-transform: uppercase; color: ${dark ? '#adb5bd' : '#495057'}; }
            #__raregems_collection_widget .__rg_value { margin-top: 8px; font-size: 16px; font-weight: bold; }
            #__raregems_collection_widget .__rg_value img { display: inline-block; margin-right: 4px; vertical-align: -2px; }
        </style>
        <div class="__rg_header">
            Tradable on
            <img src="https://s.raregems.io/img/logo${dark ? '_white' : '_black'}.svg" width="100" height="21" alt="RareGems">
        </div>
    ...
    items[0].lastElementChild.textContent = Number(r.tokens_count)
    items[1].lastElementChild.textContent = Number(r.owners_count)
    items[2].lastElementChild.textContent = Number(r.offers_count)

    items[3].lastElementChild.innerHTML = `<img src="https://s.raregems.io/img/chains/${chain}.svg" width="16" height="16">`
    items[3].lastElementChild.append(Number(r.floor_price))
